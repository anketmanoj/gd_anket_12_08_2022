...
class AwsAnketS3 {
  ...
  static Future<String?> uploadFile({
    ...
    final File fileVal = File(file.path);
    final endpoint = 'https://$bucket.s3.amazonaws.com';
    final uploadDest = '$destDir/${filename ?? path.basename(fileVal.path)}';

    final stream = http.ByteStream(Stream.castFrom(fileVal.openRead()));
    final length = await fileVal.length();

    final uri = Uri.parse(endpoint);
    final req = http.MultipartRequest("POST", uri);
    final multipartFile = http.MultipartFile('file', stream, length,
        filename: path.basename(fileVal.path));

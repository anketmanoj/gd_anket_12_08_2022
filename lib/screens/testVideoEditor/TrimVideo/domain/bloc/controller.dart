...
class VideoEditorController extends ChangeNotifier {
  ...
  Future<String> _getCrop() async {
    ...
    if (enddx > _videoWidth) enddx = 1080;
    if (enddy > _videoHeight) enddy = 1920;
    ...
    await FFprobeKit.execute(
      ...
      value.getOutput().then((mapOutput) async {
        ...
        if (double.parse(durationString) -
          ...
          String command =
              " -i ${vidFilePath} -ss ${formatTime((selectedCoverVal!.timeMs.toDouble() / 1000).toInt())}  -t ${formatTime((selectedCoverVal!.timeMs.toDouble() / 1000).toInt() + 4)} -vf \"fps=25,scale=400000:-1\" -loop 0 -y ${outputPath}";
          ...
          String command =
              " -i ${vidFilePath} -ss ${formatTime((selectedCoverVal!.timeMs.toDouble() / 1000).toInt())}  -t ${formatTime(duration.toInt())} -vf \"fps=25,scale=400000:-1\" -loop 0 -y ${outputPath}";
          log(command);
          await FFmpegKit.execute(command).then((value) {
            log("done with gif");
          });
